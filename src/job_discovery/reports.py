    ...
    for job in jobs:
        lines.extend(
            [
                ...
                f"- Score: {job['score']}",
                f"- Preference fit: {job['preference_score']}",
                f"- Skills match: {job['resume_score']}",
                f"- Eligibility fit: {job['screening_score']}",
                f"- Source: {job['source']}",
                f"- Match reasons: {'; '.join(job['match_reasons']) or 'None'}",
                f"- Rejection reasons: {'; '.join(job['rejection_reasons']) or 'None'}",
                f"- Skills evidence: {'; '.join(job['resume_reasons']) or 'None'}",
                f"- Skills gaps: {'; '.join(job['resume_gaps']) or 'None'}",
                f"- Eligibility notes: {'; '.join(job['screening_reasons']) or 'None'}",
                f"- Eligibility flags: {'; '.join(job['screening_flags']) or 'None'}",
